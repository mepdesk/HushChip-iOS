...
struct BackupWizardView: View {
    ...
    private var introContent: some View {
        VStack(spacing: 0) {
            ...
            Text("Create an encrypted copy of all your secrets on a second HushChip card. You will need two cards.")
        ...
        HStack(spacing: 0) {
            ForEach(BackupWizardStep.allCases, id: \.rawValue) { step in
                if step.rawValue > 0 {
                    // Connecting line
                    Rectangle()
                        .fill(step.rawValue <= currentStep.rawValue ? Color.hcBorderHover : Color.hcBorder)
                        .frame(height: 1)
                }
                ...
                VStack(spacing: 6) {
                    ZStack {
                        Circle()
                            .fill(step.rawValue <= currentStep.rawValue ? Color.hcBorderHover : Color.clear)
                            .overlay(
                                Circle()
                                    .stroke(step.rawValue <= currentStep.rawValue ? Color.hcBorderHover : Color.hcBorder, lineWidth: 1)
                            )
                    ...
                    Text(step.label)
                        ...
                        .foregroundColor(step.rawValue <= currentStep.rawValue ? Color.hcTextMuted : Color.hcTextGhost)
                }
            }
        }
    ...
    private var cardIcon: some View {
        RoundedRectangle(cornerRadius: 8)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(Color.hcTextFaint, lineWidth: 1)
            )
            .overlay(
                // Mini chip
                RoundedRectangle(cornerRadius: 2)
                    .fill(Color.hcTextFaint.opacity(0.4))
