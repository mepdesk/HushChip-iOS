...
//
//  PinCodeView.swift
//  HushChip — PIN unlock screen (existing card)
    ...
    var body: some View {
        HStack(spacing: 8) {
            ForEach(0..<slotCount, id: \.self) { i in
                ...
                Circle()
                    .fill(filled
                          ? (wrongFlash ? Color.hcDanger : Color.hcTextMuted)
                          : Color.clear)
                    .overlay(
                        Circle().stroke(
                            filled
                                ? Color.clear
                                : (wrongFlash ? Color.hcDanger.opacity(0.5) : Color.hcBorder),
                            lineWidth: 1
                        )
                    )
            ...
            VStack(spacing: 0) {
                ...
                if cardState.consecutiveWrongPins > 0 {
                    ...
                    Text("\(remaining) attempt\(remaining == 1 ? "" : "s") remaining")
                        ...
                        .foregroundColor(remaining <= 3 ? .hcDanger : .hcTextFaint)
                        .transition(.opacity)
