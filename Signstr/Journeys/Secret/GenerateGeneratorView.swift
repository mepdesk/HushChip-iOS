    ...
    var body: some View {
        HStack {
            if isSecure && !showText {
                SecureField("", text: $text, prompt: Text(placeholder).foregroundColor(.sgTextGhost))
                    .font(.custom("Outfit-Light", size: 13))
                    .foregroundColor(.sgTextBright)
            } else {
                TextField("", text: $text, prompt: Text(placeholder).foregroundColor(.sgTextGhost))
                    .font(.custom("Outfit-Light", size: 13))
                    .foregroundColor(.sgTextBright)
...
struct GenerateGeneratorView: View {
        ...
        HStack(spacing: 0) {
            ...
            Button(action: {
                ...
                Text("GENERATE")
                    ...
                    .tracking(2)
                    .foregroundColor(isGenerate ? .sgTextBright : .sgTextFaint)
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 10)
                    .background(isGenerate ? Color.sgBorderHover : Color.sgBgRaised)
            }
            Button(action: {
                generatorModeNavData = GeneratorModeNavData(
                    generatorMode: generatorModeNavData.generatorMode,
            ...
            HStack(spacing: 0) {
                ForEach([12, 18, 24], id: \.self) { count in
                    Button(action: {
                        ...
                        Text("\(count)")
                            .font(.custom("Outfit-Regular", size: 12))
                            .foregroundColor(selectedWordCount == count ? .sgTextBright : .sgTextFaint)
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, 10)
                            .background(selectedWordCount == count ? Color.sgBorderHover : Color.sgBgRaised)
                    }
                }
            }
            .overlay(
    ...
    private func wordCountButton(size: MnemonicSize, label: String) -> some View {
        Button(action: {
            ...
            Text(label)
                .font(.custom("Outfit-Regular", size: 12))
                .foregroundColor(mnemonicSizeOptions.selectedOption == size ? .sgTextBright : .sgTextFaint)
                .frame(maxWidth: .infinity)
                .padding(.vertical, 10)
                .background(mnemonicSizeOptions.selectedOption == size ? Color.sgBorderHover : Color.sgBgRaised)
        }
    }
    ...
    private var exportRightsToggle: some View {
        HStack {
            ...
            Toggle("", isOn: $allowPlaintextExport)
                .toggleStyle(SwitchToggleStyle(tint: .sgBorderHover))
                .labelsHidden()
        }
        .padding(.vertical, 4)
    }
